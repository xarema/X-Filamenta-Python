# Junie (JetBrains) — Prompt 02: Nettoyage repo + docs + .gitignore (Code mode)

CONSIGNES JUNIE
- Utilise **Code mode**.
- Lis d’abord `.github/` et respecte strictement les règles IA / conventions.
- Pour les suppressions: prudence maximale, justification obligatoire.
- Vérifie que la CI / scripts ne cassent pas (exécute test/lint/build si possible).
...
SPÉCIFIQUE JUNIE
- Applique le nettoyage étape par étape, avec vérification entre étapes.
- Si tu dois déplacer beaucoup de fichiers, fais-le par lots, en corrigeant les imports/liens à chaque lot.