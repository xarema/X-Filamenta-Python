# Junie (JetBrains) — Prompt 01: Audit sécurité/qualité + corrections (Code mode)

CONSIGNES JUNIE
- Utilise **Code mode**.
- Commence par lire `.github/` (règles IA, workflows, conventions). Applique-les strictement.
- Tu peux exécuter des commandes (tests/lint/build) mais évite toute commande destructive.
- Après chaque série de changements, exécute une vérification minimale (lint/test/build) si disponible.
...
SPÉCIFIQUE JUNIE (EXECUTION)
- Propose un plan multi-étapes, puis exécute-le.
- Fais des commits atomiques si possible (ou étapes clairement séparées) et vérifie à chaque étape.